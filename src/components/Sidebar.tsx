...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const handleVesselToggle = (vesselId: string) => {
    const newSelection = selectedVessels.includes(vesselId)
  ...
  const handleSelectAll = () => {
    const allVesselIds = vessels.map(v => v.id);
    onVesselSelectionChange(selectedVessels.length === vessels.length ? [] : allVesselIds);
  };
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Fleet</h3>
            <button
          ...
          <div className="space-y-2">
            {vessels.map((vessel) => (
              <div
                key={vessel.id}
                className="flex items-center space-x-3 px-3 py-2 rounded hover:bg-slate-700 transition-colors"
              >
                <Checkbox
                ...
                />
                <div className={`w-2 h-2 rounded-full opacity-60 ${vessel.active ? 'bg-yellow-500' : 'bg-slate-400'}`}></div>
                <span className="text-sm flex-1">{vessel.name}</span>
              </div>
            ))}
          </div>
        </div>
